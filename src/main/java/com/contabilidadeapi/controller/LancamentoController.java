...
import com.contabilidadeapi.service.LancamentoService;
...
public class LancamentoController {
    ...
    @GetMapping
    public List<LancamentoResponseDTO> listar(@RequestParam(required = false) String descricao, @RequestParam(required = false) Long categoriaId, @RequestParam(required = false) Long centroCustoId) {
        ...
        if (centroCustoId != null) {
            resultado = resultado.stream().filter(item -> centroCustoId.equals(item.getCentroCustoId())).collect(java.util.stream.Collectors.toList());
        }
        return resultado;
    }
